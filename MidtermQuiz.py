...
#Function parameters are the names listed in the functions definition. Function argument is when the real values passed to the function.
# 2. In your own words, describe what each of the following errors means.
# Write your response using complete sentences.
#A syntax error character or string placed in the wrong command or giving the wrong instruction that cause failure in the execution.
#A type error is an operand or argument passed to a function that is incompatible with the type of operater or function.
#A name error is when a syntax omits double quotation marks.
# syntax error
# type error
# name error
...
# a string data type?
# You would use the str()function.
...
# A variable name cannot start with a digit. A variable name can only contain alpha numeric characters and underscores. Ther is no limit on the length of the variable name.
...
#= This is of the equality operater.
#== is from the comparison operater.
#=! non-null assertion operater.
#symbols
# = 
# == 
# =!
...
import switch
options=("20mph" "30mph" "40mph" "70mph")
running=True

while running:

 driver=None
 gears=switch(options)

while driver not in options:
   driver = input("Enter a gear(20mph,30mph,40mph,70mph): ")

   print(f"driver:{driver}")
   print(f"gears:{gears}")
if driver == gears:
print(you have reached "20mph" please switch gears)
